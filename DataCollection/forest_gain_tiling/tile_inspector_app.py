...
REJECTION_COLOR = {
    1: "#4a4a4a",  # no_land
    2: "#e07b39",  # insufficient_veg
    3: "#c0392b",  # no_forest_gain
    4: "#8e44ad",  # missing_imagery
}
VALID_COLOR = "#27ae60"
GRID_STEP = 0.25
GRID_LON_MIN = -180.0
...
def load_product_extent() -> tuple[dict, object, tuple[float, float, float, float]]:
    """Load the real, non-rectangular Forest Gain product footprint."""
...
def load_aoi_checkpoint(period: str) -> tuple[list[dict], list[dict]]:
    """Valid/rejected AOI records for `period`, flattened to properties dicts.

    Assumes aoi_filter_checkpoint_<period>.json is {"valid": [...], "rejected":
    [...]}, matching the two lists the offline aoi_map_<period>.py script
    reads separately -- adjust the two key names below if the checkpoint's
...
def build_aoi_geojson(period: str):
    """Build valid/rejected AOIs as WGS84 GeoJSON for Leaflet/Folium."""
...
def run_worker(action: str, period: str, payload: dict) -> dict:
    """Use the existing period-specific code in an isolated process."""
...
point = st.session_state.get(
    "selected_point", ((west + east) / 2, (south + north) / 2)
)
...
map_ = folium.Map(location=[point[1], point[0]], zoom_start=4, control_scale=True)
folium.GeoJson(
    extent_geojson,
